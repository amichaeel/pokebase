...
const HMMovesTable = ({ pokemonData }) => {
  const [moves, setMoves] = useState([]);
  const [loading, setLoading] = useState(true);
  const [sortConfig, setSortConfig] = useState({ key: 'hm', direction: 'asc' });

  useEffect(() => {
    const fetchHMMoves = async () => {
      setLoading(true);
        ...
        pokemonData.moves
          ...
          .map(async (move) => {
            ...
            if (moveDetail.machines.length > 0) {
              ...
              if (machineDetail.item.name.startsWith('hm')) {
                const hmNumber = machineDetail.item.name.match(/\d+/)[0];
                return {
                  hm: `HM${hmNumber}`,
  ...
  return (
    <div className="w-full">
      {loading ? (
        ...
        <div className="text-center">{capitalize(pokemonData.name)} cannot learn any moves by HM.</div>
      ) : (
        <div className="table-wrapper">
          <table className="w-full text-left text-sm">
            <thead className="sticky-header">
              <tr className="hover:*:bg-black/10 hover:*:cursor-pointer">
                <th onClick={() => handleSort('hm')} className={`py-2 ${sortConfig.key === 'hm' ? 'bg-blue-100' : ''}`}>
