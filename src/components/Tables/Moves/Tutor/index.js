...
const TutorMovesTable = ({ pokemonData }) => {
  const [moves, setMoves] = useState([]);
  const [loading, setLoading] = useState(true);
  const [sortConfig, setSortConfig] = useState({ key: 'name', direction: 'asc' });

  useEffect(() => {
    const fetchTutorMoves = async () => {
      setLoading(true);
  ...
  return (
    <div className="w-full">
      {loading ? (
        ...
        <div className="text-center">{capitalize(pokemonData.name)} cannot learn any moves by tutor.</div>
      ) : (
...
export default TutorMovesTable;
