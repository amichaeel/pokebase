        ...
        <table className="table max-w-6xl">
          <thead>
            <tr className='hover:*:bg-black/10 hover:*:cursor-pointer'>
              <th className={` ${sortConfig.key === '#' ? 'bg-blue-100' : ''}`} onClick={() => handleSort('#')}>
            ...
            {filteredData.map((pokemon) => (
              <tr className='hover:bg-black/10' key={pokemon.id}>
                ...
                <td className="flex items-center space-x-2">
                  ...
                  <div>
                    <span className='hover:underline hover:cursor-pointer' onClick={() => router.push(`/pokedex/pokemon/${pokemon.name}`)}>
                      {pokemon.species.name.replace('-', ' ').split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}
                    </span>
                    {pokemon.species.name !== pokemon.name && <div className="text-sm text-gray-500">{pokemon.name.replace('-', ' ').split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}</div>}
                  </div>
                </td>
