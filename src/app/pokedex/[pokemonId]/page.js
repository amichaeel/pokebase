  ...
  useEffect(() => {
    ...
    const identifySpecies = async (species) => {
      try {
        ...
        const matchIndex = pokedex.pokemon_entries.findIndex(
          (entry) =>
            entry.pokemon_species.name === species.toLowerCase() ||
            entry.entry_number == species,
        );
    ...
    const fetchPokemonData = async (url, isDefault) => {
      ...
      setAllPokemonData((prevData) => {
        const alreadyExists = prevData.some(
          (pokemon) => pokemon.id === pokemonObj.id,
        );
        if (alreadyExists) {
          return prevData;
        }
  ...
  return (
    <div className="flex w-full justify-center">
      <div className="flex space-y-6 flex-col w-full max-w-6xl">
        <div
          className={`flex flex-col w-full lg:rounded-b-3xl rounded-none items-center justify-between`}
          style={{
            background: typeGradients[selectedPokemonData.types[0].type.name],
          }}
        >
          <div className="pb-4 grid grid-cols-3 w-full">
            ...
            <div className="flex col-span-1 w-full justify-center">
              <h1 className="md:text-4xl text-white/90 text-2xl py-3 w-full font-bold text-center">
                {capitalizeWords(defaultPokemonData.species.name)}
              </h1>
            </div>
          ...
          <div
            role="tablist"
            className="tabs flex gap-y-1 *:transition-all flex-wrap w-full"
          >
            {Object.keys(allPokemonData).length > 1 &&
              allPokemonData.map((pokemon, index) => {
                return (
                  <a
                    onClick={() =>
                      setSelectedPokemonData(allPokemonData[index])
                    }
                    key={index}
                    role="tab"
            ...
            >
              ...
              {hovered && (
                <div className="absolute inset-0 flex items-center justify-center">
                  <SpeakerWaveIcon
                    onClick={playCry}
                    className="w-20 h-20 p-6 transition-all bg-white/40 hover:bg-white/70 rounded-full cursor-pointer"
                  />
                </div>
              )}
            </div>
